...
fn main() {
    ...
    let function_index = args[2]
        .parse::<usize>()
        .expect("Failed to parse function index");
    ...
    let sig_type = &module
        .type_section()
        .expect("No type section: module malformed")
        .types()[sig.type_ref() as usize];
    let code = &module
        .code_section()
        .expect("Already checked, impossible")
        .bodies()[function_index];
