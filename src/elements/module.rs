...
use super::{deserialize_buffer, serialize, Deserialize, Error, External, Serialize, Uint32};
...
impl Module {
    ...
    pub fn insert_section(&mut self, section: Section) -> Result<(), Error> {
        ...
        if sections
            .iter()
            .position(|s| s.order() == section.order())
            .is_some()
        {
